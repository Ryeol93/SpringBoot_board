...
//    사용자가 전달한 점수의 총점과 평균 점수 출력
    ...
    @GetMapping("/ex09")
    public void ex09(ExampleVO exampleVO, @RequestParam("data") String gender, Model model){
        log.info("--------------------------------------");
        log.info(exampleVO.toString());
        log.info("gender : " + gender);
        log.info("--------------------------------------");
...
//       처음에 로그인페이지로 이동하기 위해서
    }
//  똑같은 경로를 요청했더라도 메소드전송방식이 서로 다르기 때문에 알맞은 메소드가 실행된다
    @PostMapping("/login")
    public String login(/*@ModelAttribute("dto")*/ LoginDTO loginDTO) {
        return "app/task01/" + (loginDTO.getId().equals("admin") ? "admin" : "user");
//        로그인페이지에서 loginDTO파라미터를 받고 (loginDTO.getId().equals("admin")면 admin페이지로 아니면 user페이지로이동
    }
// 실습2
    /*
     ...
     * */
    @GetMapping("/checkIn")
    public String checkIn(){ return "app/task02/checkIn"; }
//    checkIn을 요청하면 아래 경로로 간다
    ...
    public String getToWork(@ModelAttribute("name") String name){
        ...
        if(lateCondition){
            return "app/task02/late";
        }
        return "app/task02/getToWork";
    }
    ...
    public String leaveWork(@ModelAttribute("name") String name){
        ...
        if(leaveWorkCondition){
            return "app/task02/leaveWork";
        }
        return "app/task02/work";
    }
