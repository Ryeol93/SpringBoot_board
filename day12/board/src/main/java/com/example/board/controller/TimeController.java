...
public class TimeController {
    ...
    @GetMapping("/time")
    @ResponseBody //일반컨트롤러에서 rest방식을 사용하기 위해서는 @ResponseBody를 써준다.
    // 일반 컨트롤러에서 html 이동이 아닌 값을 그대로 전달하기 위해서는 @ResponseBody를 사용한다.

//    외부에서 댓글을 작성한 시간을 문자열로 전달받는다.
    public String getReplyDate(String replyDate) throws ParseException {
        //전달받은 문자열 형식(yyyy-MM-dd HH:mm:ss)을 작성한다.
...
//            1의 자리 수는 앞에 0을 붙인다. ex) 13시 '0'4분
            time = (h < 10 ? "0" : "") + h + ":" + (mm < 10 ? "0" : "") + mm + ":" + (s < 10 ? "0" : "") + s;
        }else{ //년 월 일
//            1의 자리 수는 앞에 0을 붙인다.
            time = y + "-" + (m < 10 ? "0" : "") + m + "-" + (d < 10 ? "0" : "") + d;
        }
        //@ResponseBody써줌으로서 html로 가지않고 time값이 그대로 리턴된다.
        return time;
    }
}